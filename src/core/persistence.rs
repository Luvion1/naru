...
pub enum PersistenceError {
    ...
    #[error("Encryption key not set. Please set NARU_ENCRYPTION_KEY environment variable.")]
...
pub fn init_project() -> Result<(), PersistenceError> {
    if Path::new(NARU_DIR).exists() {
        return Err(PersistenceError::ProjectAlreadyInitialized);
    }
    fs::create_dir(NARU_DIR)?;

    let mut environments = HashMap::new();
    for env in &["development", "staging", "production"] {
        environments.insert(env.to_string(), EnvironmentConfig { entries: HashMap::new() });
    }

    let config = ConfigFile {
        project_name: "My Naru Project".to_string(),
...
pub fn save_json<T: serde::Serialize>(filename: &str, data: &T) -> Result<(), PersistenceError> {
    ...
    let sanitized_filename = security::sanitize_file_path(filename)
    ...
    let _lock = locking::FileLock::acquire_exclusive(&path)
        .map_err(|e| PersistenceError::IoError {
            source: std::io::Error::new(std::io::ErrorKind::Other, format!("Could not acquire file lock: {}", e))
        })?;

    let json = serde_json::to_string_pretty(data)?;
    fs::write(path, json)?;

    // Lock will be released when _lock goes out of scope
    Ok(())
}
...
pub fn import_from_env(file_path: &str, env: &str) -> Result<ConfigFile, PersistenceError> {
    ...
    let sanitized_path = security::sanitize_file_path(file_path)
        .map_err(|e| PersistenceError::IoError {
            source: std::io::Error::new(std::io::ErrorKind::InvalidInput, e)
        })?;

    // Validate environment name
    security::validate_environment_name(env)
        .map_err(|e| PersistenceError::IoError {
            source: std::io::Error::new(std::io::ErrorKind::InvalidInput, e)
        })?;
    ...
    let mut dotenv_pairs: HashMap<String, String> = HashMap::new();
    ...
    let config: ConfigFile = load_json(CONFIG_FILE)?;
    merge_map_into_config(config, env, dotenv_pairs)
}
...
pub fn import_from_yaml(file_path: &str, env: &str) -> Result<ConfigFile, PersistenceError> {
    ...
    security::check_file_size(&sanitized_path, 1024 * 1024)
        .map_err(|e| PersistenceError::IoError {
            source: std::io::Error::new(std::io::ErrorKind::InvalidData, e)
        })?;
    ...
    let config: ConfigFile = load_json(CONFIG_FILE)?;
    merge_map_into_config(config, env, string_pairs)
}
...
// Internal helper to encrypt an entry's value
fn encrypt_entry_value(entry: &mut ConfigValueEntry) -> Result<(), PersistenceError> {
    if entry.is_secret && !entry.encrypted {
        let encryption_key = get_encryption_key()?;
        let encrypted_value = crypto::encrypt_data(&entry.value, &encryption_key)
            .map_err(|e: Box<dyn std::error::Error>| PersistenceError::IoError {
...
fn merge_map_into_config(
    ...
    // Load schema
    let schema: SchemaFile = load_json(SCHEMA_FILE).unwrap_or_else(|_| SchemaFile {
        version: "1.0".to_string(),
        fields: vec![],
    });
    ...
    if !config.environments.contains_key(env) {
        config.environments.insert(env.to_string(), EnvironmentConfig { entries: HashMap::new() });
    }
    ...
    if let Some(env_config) = config.environments.get_mut(env) {
        for (key, value) in pairs {
            // Validate the key
            security::validate_config_key(&key)
                .map_err(|e| PersistenceError::IoError {
                    source: std::io::Error::new(std::io::ErrorKind::InvalidInput, e)
                })?;
            ...
            if let Some(field) = schema.fields.iter().find(|f| f.key == key) {
                ...
                crate::core::validation::validate_value(&sanitized_value, field)
                    .map_err(|e| PersistenceError::ValidationError(format!("Field '{}': {}", key, e)))?;
            }
            ...
            // Automatic encryption if it's a secret
...
pub fn export_to_env(config: &ConfigFile, env: &str, file_path: &str) -> Result<(), PersistenceError> {
    // Validate environment name
    security::validate_environment_name(env)
        .map_err(|e| PersistenceError::IoError {
            source: std::io::Error::new(std::io::ErrorKind::InvalidInput, e)
    ...
    let env_config = config.environments.get(env)
        .ok_or_else(|| PersistenceError::IoError {
            source: std::io::Error::new(std::io::ErrorKind::NotFound, format!("Environment '{}' not found", env))
        })?;
...
pub fn export_to_yaml(config: &ConfigFile, env: &str, file_path: &str) -> Result<(), PersistenceError> {
    // Validate environment name
    security::validate_environment_name(env)
        .map_err(|e| PersistenceError::IoError {
            source: std::io::Error::new(std::io::ErrorKind::InvalidInput, e)
...
fn get_encryption_key() -> Result<[u8; 32], PersistenceError> {
    let key_str = env::var("NARU_ENCRYPTION_KEY").map_err(|_| PersistenceError::MissingEncryptionKey)?;
...
pub fn encrypt_if_needed(config: &mut ConfigFile, env: &str, key: &str) -> Result<(), PersistenceError> {
    if let Some(env_config) = config.environments.get_mut(env) {
...
pub fn decrypt_if_needed(config: &mut ConfigFile, env: &str, key: &str) -> Result<(), PersistenceError> {
    if let Some(env_config) = config.environments.get_mut(env) {
        if let Some(entry) = env_config.entries.get_mut(key) {
            if entry.encrypted {
                let encryption_key = get_encryption_key()?;
                let decrypted_value = crypto::decrypt_data(&entry.value, &encryption_key)
                    .map_err(|e: Box<dyn std::error::Error>| PersistenceError::IoError {
                        source: std::io::Error::new(std::io::ErrorKind::Other, e.to_string())
                    })?;
