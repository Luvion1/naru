...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Init => {
            println!("Initializing Naru project...");
            match persistence::init_project() {
                Ok(_) => println!("Success! Initialized .naru directory."),
        ...
        Commands::Set { kv, env, secret } => {
            ...
            let schema: SchemaFile = persistence::load_json(SCHEMA_FILE)
                .unwrap_or_else(|_| {
                    ...
                    SchemaFile { version: "1.0".to_string(), fields: vec![] }
                });
            ...
            if let Some(field) = schema.fields.iter().find(|f| f.key == key) {
                ...
                entry_to_validate.validate(field)
            ...
            let old_value = old_entry.map(|entry| entry.value.clone());
            let is_previously_secret = old_entry.map_or(false, |e| e.is_secret);

            // Insert the new value
            env_config.entries.insert(key.to_string(), ConfigValueEntry {
            ...
            let log_old = if is_secret || is_previously_secret { 
                if old_value.is_some() { Some("********") } else { None }
            } else { 
                old_value.as_deref()
            };
            ...
            println!("Set '{}' = '{}' in environment '{}'", key, value, env);
        }
        Commands::Get { key, env } => {
            ...
            let env_config = config.environments.get(&env)
                .ok_or_else(|| anyhow::anyhow!("Environment '{}' not found.", env))?;
        ...
        Commands::List { env } => {
            ...
            println!("\n{} {}", Emoji("📁", ""), style(format!("Environment: {}", env)).bold().cyan());
            println!("{}", style("=".repeat(60)).dim());
            
            let mut all_keys: std::collections::BTreeSet<String> = env_config.entries.keys().cloned().collect();
            for field in &schema.fields {
                all_keys.insert(field.key.clone());
            }

            if all_keys.is_empty() {
                println!("{}", style("  (No entries found)").italic().dim());
            } else {
                for key in all_keys {
                    let entry = env_config.entries.get(&key);
                    let field_def = schema.fields.iter().find(|f| f.key == key);
                    
                    let key_style = if entry.is_some() { style(&key).bold() } else { style(&key).dim() };
                    let secret_icon = if entry.map_or(false, |e| e.is_secret) { Emoji("🔒 ", "") } else { Emoji("   ", "") };
                    ...
                    let type_str = match (entry, field_def) {
                        (_, Some(f)) => style(format!("({})", f.r#type)).dim().to_string(),
                        (Some(e), None) => style(format!("({})", e.r#type)).dim().to_string(),
                        _ => "".to_string(),
                    };

                    println!("  {} {:<20} = {:<15} {}", secret_icon, key_style, value_str, type_str);
        ...
        Commands::Import { file_path, env } => {
            ...
            let config = if file_path.ends_with(".env") {
                ...
                return Err(anyhow::anyhow!("Unsupported file format. Supported formats: .env, .yaml, .yml, .json"));
            };
            ...
            // Audit Log
            let log_path = format!("{}/audit.log", NARU_DIR);
            if let Err(e) = crate::core::audit::log_action("IMPORT", &env, None, None, Some(&file_path), &log_path) {
                eprintln!("Warning: Failed to log audit entry: {}", e);
            }

            println!("Successfully imported from {} to environment '{}'", file_path, env);
        }
        Commands::Export { file_path, env, format } => {
            // Validate environment name
            crate::core::security::validate_environment_name(&env)
                .map_err(|e| anyhow::anyhow!("Invalid environment name: {}", e))?;
            ...
            match format.as_str() {
                "env" => {
                    ...
                    println!("Successfully exported environment '{}' to {} in .env format", env, file_path);
                }
                "yaml" | "yml" => {
                    persistence::export_to_yaml(&config, &env, &file_path)?;
                    println!("Successfully exported environment '{}' to {} in YAML format", env, file_path);
                }
                _ => {
        ...
        Commands::Schema { action } => {
            let log_path = format!("{}/audit.log", NARU_DIR);
            match action {
                cli::parser::SchemaAction::Add { key, r#type, description, secret } => {
                    let field_def = if let Some(key) = key {
                        // Validate key
                        crate::core::security::validate_config_key(&key)
                            .map_err(|e| anyhow::anyhow!("Invalid field key: {}", e))?;

                        core::models::FieldDefinition {
                            key,
                            r#type,
                            description,
                            validation: None,
                    ...
                    // Audit Log
                    if let Err(e) = crate::core::audit::log_action("SCHEMA_ADD", "schema", Some(&key_name), None, None, &log_path) {
                        eprintln!("Warning: Failed to log audit entry: {}", e);
                    }
                },
                cli::parser::SchemaAction::Remove { key } => {
                    ...
                    // Audit Log
                    if let Err(e) = crate::core::audit::log_action("SCHEMA_REMOVE", "schema", Some(&key), None, None, &log_path) {
                        eprintln!("Warning: Failed to log audit entry: {}", e);
                    }
                },
                cli::parser::SchemaAction::View => {
                    let schema: SchemaFile = persistence::load_json(SCHEMA_FILE)
                        .unwrap_or_else(|_| { eprintln!("Warning: Could not load schema file, using default schema"); SchemaFile { version: "1.0".to_string(), fields: vec![] } });
                    ...
                    if schema.fields.is_empty() {
                        ...
                        println!("Fields:");
                        for field in &schema.fields {
                            println!("  - {}: {} ({})", field.key, field.r#type,
                ...
                cli::parser::SchemaAction::Edit { key } => {
                    ...
                    let existing_field = schema.fields.iter().find(|f| f.key == key)
                        .ok_or_else(|| anyhow::anyhow!("Field '{}' not found in schema", key))?;
                    ...
                    // Audit Log
                    if let Err(e) = crate::core::audit::log_action("SCHEMA_EDIT", "schema", Some(&key_name), None, None, &log_path) {
                        eprintln!("Warning: Failed to log audit entry: {}", e);
                    }
                },
            }
            ...
            match action {
                cli::parser::EnvAction::Add { name } => {
                    ...
                    if config.environments.contains_key(&name) {
                        eprintln!("Environment '{}' already exists", name);
                        return Ok(());
                    }

                    // Add new environment
                    config.environments.insert(name.clone(), EnvironmentConfig { entries: std::collections::HashMap::new() });
                    persistence::save_json(CONFIG_FILE, &config)?;

                    // Audit Log
                    let log_path = format!("{}/audit.log", NARU_DIR);
                    if let Err(e) = crate::core::audit::log_action("ENV_ADD", &name, None, None, None, &log_path) {
                        eprintln!("Warning: Failed to log audit entry: {}", e);
                    }
                ...
                cli::parser::EnvAction::Remove { name } => {
                    ...
                    if ["development", "staging", "production"].contains(&name.as_str()) {
                        eprintln!("Cannot remove default environment '{}'", name);
                        return Ok(());
                    }

                    // Check if environment exists
                    if !config.environments.contains_key(&name) {
                        eprintln!("Environment '{}' does not exist", name);
                        return Ok(());
                    }
                    ...
                    // Audit Log
                    let log_path = format!("{}/audit.log", NARU_DIR);
                    if let Err(e) = crate::core::audit::log_action("ENV_REMOVE", &name, None, None, None, &log_path) {
                        eprintln!("Warning: Failed to log audit entry: {}", e);
                    }
        ...
        Commands::Backup { action } => {
            match action {
                cli::parser::BackupAction::Create { file_path } => {
                    ...
                    std::fs::write(sanitized_path.to_str().ok_or_else(|| anyhow::anyhow!("Invalid file path"))?, json_data)?;
                ...
                cli::parser::BackupAction::Restore { file_path } => {
                    ...
                    let content = std::fs::read_to_string(sanitized_path.to_str().ok_or_else(|| anyhow::anyhow!("Invalid file path"))?)?;
                    ...
                    // Audit Log
                    let log_path = format!("{}/audit.log", NARU_DIR);
                    if let Err(e) = crate::core::audit::log_action("BACKUP_RESTORE", "all", None, None, Some(&file_path), &log_path) {
                        eprintln!("Warning: Failed to log audit entry: {}", e);
                    }
        ...
        Commands::Diff { env1, env2 } => {
            ...
            if !config.environments.contains_key(&env1) {
                eprintln!("Environment '{}' does not exist", env1);
                return Ok(());
            }

            if !config.environments.contains_key(&env2) {
                eprintln!("Environment '{}' does not exist", env2);
                return Ok(());
            }
            ...
            // Keys only in env1
            println!("Keys only in '{}':", env1);
            let mut only_in_env1 = false;
            for (key, entry) in &env1_config.entries {
                if !env2_config.entries.contains_key(key) {
                    println!("  + {}: {} ({})", key, entry.value, entry.r#type);
                    only_in_env1 = true;
                }
            }
            if !only_in_env1 {
            ...
            for (key, entry) in &env2_config.entries {
            ...
            for (key, entry1) in &env1_config.entries {
                if let Some(entry2) = env2_config.entries.get(key) {
                    if entry1.value != entry2.value {
                        println!("  ~ {}: {} -> {} ({} type)", key, entry1.value, entry2.value, entry1.r#type);
        ...
        Commands::Convert { from_file, to_file, from_format, to_format } => {
            use crate::core::formats::{ConfigFormat, JsonFormat, TomlFormat, PropertiesFormat};
            ...
            let config = match from_format.as_str() {
                "json" => {
                    let content = std::fs::read_to_string(sanitized_from_file.to_str().ok_or_else(|| anyhow::anyhow!("Invalid source file path"))?)?;
            ...
            match to_format.as_str() {
                "json" => {
                    let format = JsonFormat;
                    crate::core::formats::save_config_as_format(sanitized_to_file.to_str().ok_or_else(|| anyhow::anyhow!("Invalid destination file path"))?, &config, &format)?;
                },
                "toml" => {
                    let format = TomlFormat;
                    crate::core::formats::save_config_as_format(sanitized_to_file.to_str().ok_or_else(|| anyhow::anyhow!("Invalid destination file path"))?, &config, &format)?;
                ...
                "yaml" | "yml" => {
                    ...
                    std::fs::write(sanitized_to_file.to_str().ok_or_else(|| anyhow::anyhow!("Invalid destination file path"))?, yaml_content)?;
                },
                _ => {
            ...
            println!("Configuration converted from {} to {} and saved to {}", from_format, to_format, to_file);
        }
        Commands::Crypto { action } => {
            ...
            if key_str.len() < 32 {
                return Err(anyhow::anyhow!("Encryption key must be at least 32 characters long"));
            }
            ...
            match action {
                cli::parser::CryptoAction::Encrypt { input_file, output_file } => {
                    // Sanitize file paths to prevent directory traversal
                    let sanitized_input_file = crate::core::security::sanitize_file_path(&input_file)
                        .map_err(|e| anyhow::anyhow!("Invalid input file path: {}", e))?;

                    let sanitized_output_file = crate::core::security::sanitize_file_path(&output_file)
                        .map_err(|e| anyhow::anyhow!("Invalid output file path: {}", e))?;

                    crypto::encrypt_file(sanitized_input_file.to_str().ok_or_else(|| anyhow::anyhow!("Invalid input file path"))?,
                                         sanitized_output_file.to_str().ok_or_else(|| anyhow::anyhow!("Invalid output file path"))?,
                                         &key)
                        .map_err(|e| anyhow::anyhow!("Failed to encrypt file: {}", e))?;
                    println!("File encrypted successfully from {} to {}", input_file, output_file);
                },
                cli::parser::CryptoAction::Decrypt { input_file, output_file } => {
                    // Sanitize file paths to prevent directory traversal
                    let sanitized_input_file = crate::core::security::sanitize_file_path(&input_file)
                        .map_err(|e| anyhow::anyhow!("Invalid input file path: {}", e))?;
                    ...
                    crypto::decrypt_file(sanitized_input_file.to_str().ok_or_else(|| anyhow::anyhow!("Invalid input file path"))?,
                                         sanitized_output_file.to_str().ok_or_else(|| anyhow::anyhow!("Invalid output file path"))?,
                                         &key)
                        .map_err(|e| anyhow::anyhow!("Failed to decrypt file: {}", e))?;
                    println!("File decrypted successfully from {} to {}", input_file, output_file);
                },
            }
        }
        Commands::Audit { count } => {
            ...
            let entries = AuditLogEntry::get_recent_logs(&log_path, count)
                .map_err(|e| anyhow::anyhow!("Failed to read audit log: {}", e))?;

            if entries.is_empty() {
                println!("No audit entries found.");
            } else {
                println!("Recent {} audit entries:", entries.len());
        ...
        Commands::Validate => {
            ...
            for (env_name, env_config) in &config.environments {
                println!("Checking environment '{}'...", env_name);
            ...
            if errors == 0 {
                ...
                println!("\nValidation failed with {} error(s).", errors);
            }
        }
        Commands::Version => {
            println!("Naru v0.1.0");
